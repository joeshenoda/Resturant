...
<div class="card" >
    ...
    <form method="get" id="vendorRegisterForm" action="{{route('admin.user.get')}}" aria-label="{{ __('Get') }} {{__('User')}}">
        @csrf
        ...
        <button id="submit" type="submit" class=" js-ajax btn btn-sm btn-success btn-pill px-2 py-1 fs--15 " style="float:left"><span> {{ __('Get') }} {{__('User')}}</span> <span><i class="fa
        ...
        <div class="container">
            <div class="justify-content-center">
                <div id="vendorRegister">
                    @if(isset($user))
                    <form method="POST" id="vendorRegisterForm" action="{{isset($user)?route('admin.user.update', $user):route('admin.user.store')}}" aria-label="{{ __('Register') }}">
                        @csrf
